...
public class StudyPlanService {
    ...
    public StudyPlanResponse updatePlan(UUID planId, StudyPlanRequest request, UUID userId) {
        var plan = studyPlanRepository.findById(planId)
                .orElseThrow(() -> new RuntimeException("Plan not found"));

        // ✅ Authorization check
        if (!plan.getUser().getId().equals(userId)) {
            throw new RuntimeException("Plan belongs to another user - Unauthorized");
        }
