        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          {selectedTopic && (
            <div className="animate-fadeUp">
              <AiChatPanel topicId={selectedTopic.id} topicName={selectedTopic.name} />
            </div>
          )}
        </div>
          ...
          {playlists.map((playlist) => (
            <div key={playlist.id} className="bg-[#111620]/80 backdrop-blur-xl border border-white/[0.08] rounded-[2rem] overflow-hidden shadow-2xl">
              ...
              <div className="p-6 space-y-3">
                {Array.from({ length: playlist.videoCount || 3 }).map((_, idx) => {
                  const i = idx + 1
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`font-bold ${watched ? 'text-emerald-400' : 'text-slate-300'}`}>{playlist.title} - Bolum {i}</span>
                      </div>
